...
//let finalRanges = intermediateRanges
let finalRanges = combineOverlappingRanges(intermediateRanges)
finalRanges = combineOverlappingRanges(finalRanges)
finalRanges = combineOverlappingRanges(finalRanges)
...
for (let r of finalRanges) {
	...
	const add = 1n + (r.end > r.begin ? r.end - r.begin : r.begin - r.end)
	idCount += add
	if (add < 0n) console.log(`Count after: ${idCount} r: {${r.begin}-${r.end}} add: ${add}`)
}
